...
class MemoryStore:
    ...
    def get_subgraph(self, seed_ids, depth=1):
        ...
        for _ in range(depth):
            ...
            # --- Fetch nodes ---
            # Single IN clause, no *2 multiplication, so safe up to 999 nodes.
            placeholders = ",".join(["?"] * len(frontier_list))
            query_nodes = f"SELECT * FROM beliefs WHERE id IN ({placeholders})"
            rows = self.conn.execute(query_nodes, frontier_list).fetchall()
            for r in rows:
                node = dict(r)
                all_nodes[node["id"]] = node

            # --- Fetch edges (chunked) ---
            # The edge query passes the list TWICE (source_id IN + target_id IN),
            # so each chunk consumes len(chunk)*2 variables.
            # batch_size=450 → 450*2=900, safely under SQLite's 999 limit.
            # Same pattern already used in forget_weak_beliefs() and prune_graph().
            edge_rows = []
            batch_size = 450
            for i in range(0, len(frontier_list), batch_size):
                chunk = frontier_list[i : i + batch_size]
                ph = ",".join(["?"] * len(chunk))
                query_edges = f"""
                    SELECT * FROM edges
                    ...
                    OR target_id IN ({ph})
                """
                edge_rows.extend(self.conn.execute(query_edges, chunk * 2).fetchall())
